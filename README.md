...
Objective: Classify images of food (pizza, steak, sushi) using a CNN and achieve at least 80% classification accuracy on the test set.

Dataset: Food-101, using a subset of 2,000 training and 700 test images split across 3 classes for efficiency and speed.

Performance Goal: ≥80% accuracy on test data.

Constraints: Designed to fit within 15 GB storage and run in under 2 hours on CPU (faster on GPU).​
...
How to Use
Requirements: Python, PyTorch, torchvision, scikit-learn, matplotlib, ultralytics.

Dataset: Download Food-101 via torchvision or from Kaggle, or use provided subset scripts.

Run Training: Execute the notebook Lab08.ipynb step by step.

Visualizations: Training metrics and model performance plots are saved as training_history.jpg for review.

Inference: Use the last notebook cells to test custom images.
